            <div class="record_list">
             <?php
              echo form_open(base_url().admin.'/products/manage-images',$drop_down_attributes['form']);
			 ?>
             ...
             *Select a product:&nbsp;&nbsp;&nbsp;&nbsp;<?php echo form_dropdown('product_id',$drop_down_attributes['product_id'],$product_id,"class='select_action' style='width:285px;' id='product_id' onchange=SubmitManageImageForm('".base_url().admin."/products/manage-images/'+this.value+'/$cat_id')");?>&nbsp;<span id="error1" class="error2"><?php echo form_error('product_id');?></span>
             </div>
             <?php
              if(!empty($product_id))
			  {
			   ...
			   <?php
			   $js_function = "OpenAddImageForm('".base_url().admin."/products/add-image','".$product_id."/".$cat_id."')";
             ...
             <!--Data Needed for Positioning-->
              ...
              <input type="hidden" name="file_path" value="<?php echo base_url().admin."/home/UpdatePosition/".PRODUCT_IMAGES;?>/product_id" id="file_path" />
             <?php
			 echo form_close();
			 if(count($image_list) > 0)
		     {
              echo form_open(base_url().admin.'/products/manage-images',$deletion_attribute['form']);
		     ?>
            ...
            <ul>
            	<li>
                    ...
                    <div class="ulTableinner sn-no-other" style="width:130px; text-align:center;">Image</div>  
                    <div class="ulTableinner sn-no-other" style="width:80px;  text-align:center;">Main Image</div>  
			...
			?>
			  ...
			  ?>
			    <li id="recordsArray_<?php echo $image['id'];?>" style="padding:2px;">
                 ...
                 <div class="ulTableinner_record sn-no-other-record" style="width:130px; text-align:center;">
                 <?php
				  if(!empty($image['image_file']))
				  {
				  ?>
				    <img src="<?php echo base_url();?>images/product/<?php echo $image['image_file'];?>" width="120" height="80" />
				   <?php
				  }
				  else
				  {
                 ...
                 <div class="ulTableinner_record sn-no-other-record" style="width:80px; text-align:center;">
                 
                  <?php
				   if($image['status']=='1')
				   {
				    $check = false;
				    if($image['main_image']=='1') $check = TRUE;
				    $main_image_data = array(
					 'name'    => 'main_image',
					 'id'      => 'main_image_'.$image['id'],
					 'value'   => $image['id'],
					 'checked' => $check,
					 'onclick' => 'UpdateMainImage(\''.base_url().admin.'/products/change-main-image/'.$cat_id.'/'.$product_id.'/'.$image['id'].'\')',
					 );
  		            echo form_radio($main_image_data);
				   }
				   else echo "&nbsp;";
				  ?>    
                 ...
                 <div class="ulTableinner_record sn-no-other-record" style="width:90px; text-align:center;">
                  <?php
				   if($image['main_image']=='0')
				   {
				    if($image['status'] == 0)
				    {
				     echo anchor(base_url().admin.'/products/changestatus/'.$image['id'].'/1/manage-images/'.$product_id.'/'.$cat_id, 'Show',array('title'=>'Show'))." | <span class='current_status'>Hide</span>";
				    }
				    elseif($image['status'] == 1)
				    {
				     echo "<span class='current_status'>Show</span> | ".anchor(base_url().admin."/products/changestatus/".$image['id'].'/0/manage-images/'.$product_id.'/'.$cat_id, 'Hide',array('title'=>'Hide'));
				    }
				   }
				   else echo "&nbsp;";
                  ?>
                 ...
                 <div class="ulTableinner_record sn-no-other-record" style="width:160px;  text-align:center;">&nbsp;  
                  <?php
                  $js_edit_function="OpenProductEditImageForm('".base_url().admin."/products/edit-image','".$image['id']."','".$product_id."','".$cat_id."')";
				  ?>
