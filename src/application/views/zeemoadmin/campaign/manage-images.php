            <div class="record_list">
             <?php
              echo form_open(base_url().admin.'/campaign/manage-images',$drop_down_attributes['form']);
			 ?>
             ...
             *Select a campaign:&nbsp;&nbsp;&nbsp;&nbsp;<?php echo form_dropdown('campaign_id',$drop_down_attributes['campaign_id'],$campaign_id,"class='select_action' style='width:285px;' id='campaign_id' onchange=SubmitManageImageForm('".base_url().admin."/campaign/manage-images/'+this.value+'/')");?>&nbsp;<span id="error1" class="error2"><?php echo form_error('campaign_id');?></span>
             </div>
             <?php
              if(!empty($campaign_id) and $campaign_id > 0)
			  {
			   ?>
			   ...
			   <?php
				...
				{
			     $js_function="OpenAddImageForm('".base_url().admin."/campaign/AddImage','".$campaign_id."')";
				 echo form_submit('add_image','Add Image','onclick="'.$js_function.'"'); 
				}
				else
				{
			     $js_function="OpenAddImageForm('".base_url().admin."/campaign/ImageUploader','".$campaign_id."')";
				 echo "&nbsp;&nbsp;".form_submit('add_image','Add Image','onclick="'.$js_function.'"');
				}
			   ?>
               </div>
             ...
             <!--Data Needed for Positioning-->
              <input type="hidden" name="parent_id" value="<?php echo $campaign_id;?>" id="parent_id" />
              
              <input type="hidden" name="file_path" value="<?php echo base_url().admin."/home/UpdatePosition/".CAMPAIGN_IMAGES;?>/campaign_id" id="file_path" />
             <?php
			 echo form_close();
			 if(count($image_list) > 0)
		     {
              echo form_open(base_url().admin.'/campaign/manage-images',$deletion_attribute['form']);
		     ?>
			  ...
			  ?>
			    <li id="recordsArray_<?php echo $image['id'];?>" style="padding:2px;">
                 ...
                 <?php
				  ...
				  {
				  ?>
				    <img src="<?php echo base_url();?>images/campaign/<?php echo $image['image_file'];?>" width="120" height="80" />
				   <?php
				  }
				  else
				  {
                 ...
                 <div class="ulTableinner_record sn-no-other-record" style="width:90px; text-align:center;">
                  <?php
				    ...
				    if($image['status'] == 0)
				    {
				     echo anchor(base_url().admin.'/campaign/changestatus/'.$image['id'].'/1/manage-images/'.$campaign_id, 'Show',array('title'=>'Show'))." | <span class='current_status'>Hide</span>";
				    }
				    elseif($image['status'] == 1)
				    {
				     echo "<span class='current_status'>Show</span> | ".anchor(base_url().admin."/campaign/changestatus/".$image['id'].'/0/manage-images/'.$campaign_id, 'Hide',array('title'=>'Hide'));
				    }
                 ...
                 <div class="ulTableinner_record sn-no-other-record" style="width:160px;  text-align:center;">&nbsp;  
                  <?php
                  $js_edit_function="OpenProjectEditImageForm('".base_url().admin."/campaign/edit-image','".$image['id']."','".$campaign_id."')";
				  ?>
