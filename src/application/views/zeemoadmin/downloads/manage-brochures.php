            <div class="record_list">
             ...
             <?php
              echo form_open(base_url().admin.'/downloads/managedownloads',$drop_down_attributes['form']);
			 ?>
             ...
             *Select Category :&nbsp;&nbsp;&nbsp;&nbsp;<?php echo form_dropdown('cat_id',$drop_down_attributes['cat_id'],$cat_id,"class='select_action' style='width:285px;' id='cat_id' onchange=SubmitManageImageForm('".base_url().admin."/downloads/managedownloads/'+this.value)");?>&nbsp;<span id="error1" class="error2"><?php echo form_error('cat_id');?></span>
             </div>
             <?php
              if(!empty($cat_id))
			  {
			   ...
			   <?php
			   $js_function="OpenAddBrochureForm('".base_url().admin."/downloads/AddBrochure','".$cat_id."')";
             ...
             <!--Data Needed for Positioning-->
              ...
              <input type="hidden" name="file_path" value="<?php echo base_url().admin."/home/UpdatePosition/".DOWNLOADS;?>/cat_id" id="file_path" />
             <?php
			 echo form_close();
			 if(count($brochure_list) > 0)
		     {
              echo form_open(base_url().admin.'/downloads/managedownloads',$deletion_attribute['form']);
		     ?>
           ...
           <?php
            if(count($brochure_list) > 0)
			{
			  ...
			  foreach($brochure_list as $brochure)
			  {
			   ?>
			    <li id="recordsArray_<?php echo $brochure['id'];?>" style="padding:2px;">
                 ...
                 <div class="ulTableinner_record sn-no-other-record" style="width:271px;padding:2px;"><a href="<?php echo base_url()."brochures/downloads/".$brochure['brochure_file'];?>" target="_blank"><?php echo ucwords($brochure['brochure_title']);?></a>&nbsp;</div>
                 
                 <div class="ulTableinner_record sn-no-other-record" style="width:105px;">
                  <?php
				   if($brochure['status'] == 0)
				   {
				    echo anchor(base_url().admin.'/downloads/changestatus/'.$brochure['id'].'/1/managedownloads/'.$cat_id, 'Show',array('title'=>'Show'))." | <span class='current_status'>Hide</span>";
				   }
				   elseif($brochure['status'] == 1)
				   {
				    echo "<span class='current_status'>Show</span> | ".anchor(base_url().admin.'/downloads/changestatus/'.$brochure['id'].'/0/managedownloads/'.$cat_id, 'Hide',array('title'=>'Hide'));
				   }
                  ?>
                  </div>
                 ...
                 <div class="ulTableinner_record sn-no-other-record" style="width:60px;">&nbsp;  
                  <?php
                  $js_edit_function="OpenEditBrochureForm('".base_url().admin."/downloads/EditBrochure','".$cat_id."','".$brochure['id']."')";
				  ?>
			 ...
			 {
			  ?>
			   <div class="success"> Please select download category</div>
			  <?php
			 }
			 else
			 {
